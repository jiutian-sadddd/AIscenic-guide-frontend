...
// ============================================================
// インラインシェーダーソース (Cubism 5 SDK for Web 標準)
// 外部ファイルの読み込みに失敗する問題を回避するため埋め込み
// ============================================================

const VERT_SHADER_SRC = `attribute vec2 a_position;
...
const VERT_SHADER_SRC_MASKED = `attribute vec2 a_position;
attribute vec2 a_texCoord;
varying vec2 v_texCoord;
...
void main() {
    gl_Position = u_matrix * vec4(a_position, 0.0, 1.0);
    v_texCoord = a_texCoord;
    v_clipCoord = (u_clipMatrix * vec4(a_position, 0.0, 1.0)).xy;
}`;

const VERT_SHADER_SRC_SETUP_MASK = `attribute vec2 a_position;
attribute vec2 a_texCoord;
varying vec2 v_texCoord;
varying vec2 v_clipCoord;
uniform mat4 u_matrix;
...
const FRAG_SHADER_SRC_SETUP_MASK = `precision mediump float;
varying vec2 v_texCoord;
varying vec2 v_clipCoord;
...
void main() {
    ...
    gl_FragColor = color * u_channelFlag;
}`;

const FRAG_SHADER_SRC_PREMULTIPLIED_ALPHA = `precision mediump float;
varying vec2 v_texCoord;
uniform sampler2D s_texture0;
uniform vec4 u_baseColor;
void main() {
...
const FRAG_SHADER_SRC_MASK_PREMULTIPLIED_ALPHA = `precision mediump float;
varying vec2 v_texCoord;
varying vec2 v_clipCoord;
uniform sampler2D s_texture0;
...
void main() {
    vec4 color = texture2D(s_texture0, v_texCoord) * u_baseColor;
    vec4 clipMask = texture2D(s_texture1, v_clipCoord) * u_channelFlag;
    float maskValue = clipMask.r + clipMask.g + clipMask.b + clipMask.a;
    color = color * maskValue;
    gl_FragColor = color;
}`;

const FRAG_SHADER_SRC_MASK_INVERTED_PREMULTIPLIED_ALPHA = `precision mediump float;
varying vec2 v_texCoord;
varying vec2 v_clipCoord;
uniform sampler2D s_texture0;
uniform sampler2D s_texture1;
...
void main() {
    ...
    vec4 clipMask = texture2D(s_texture1, v_clipCoord) * u_channelFlag;
    float maskValue = clipMask.r + clipMask.g + clipMask.b + clipMask.a;
    color = color * (1.0 - maskValue);
    gl_FragColor = color;
}`;

// ============================================================
// ブレンドモード用シェーダー関数定義 (main 関数を含まない)
// これらのソースは generateBlendShader() 内で連結され、
// 先頭のマクロ定義 (#define COLOR_BLEND_XXX / ALPHA_BLEND_XXX) と
// 末尾の FRAG_SHADER_SRC_BLEND (main 関数本体) の間に挿入される。
// ============================================================

/**
 * カラーブレンド関数定義 (main なし、#ifdef によるモード切替)
 *
 * 重要: 関数は常に定義される必要がある（未対応モードの場合はフォールバック）。
 *       なぜなら registerBlendShader() は全 ColorBlend × AlphaBlend の
 *       組み合わせに対してシェーダーを生成するため。
 *       一度に 1 つのマクロのみが定義されるので、#else で安全に排他可能。
 */
const FRAG_SHADER_SRC_COLOR_BLEND_FUNCTIONS = `\
vec4 BlendColor(vec4 base, vec4 blend, float factor) {
#ifdef COLOR_BLEND_NORMAL
    return blend;
#else
#ifdef COLOR_BLEND_MULTIPLY
    return base * blend;
#else
#ifdef COLOR_BLEND_SCREEN
    return vec4(1.0) - (vec4(1.0) - base) * (vec4(1.0) - blend);
#else
#ifdef COLOR_BLEND_ADD
    return base + blend;
#else
#ifdef COLOR_BLEND_DARKEN
    return min(base, blend);
#else
#ifdef COLOR_BLEND_LIGHTEN
    return max(base, blend);
#else
    // フォールバック: Normal（そのまま通過）
    return blend;
#endif
#endif
#endif
...
/** アルファブレンド関数定義 (main なし、#ifdef によるモード切替) */
const FRAG_SHADER_SRC_ALPHA_BLEND_FUNCTIONS = `\
vec4 BlendAlpha(vec4 base, vec4 blend, float factor) {
#ifdef ALPHA_BLEND_OVER
...
/** ブレンドモード用頂点シェーダー (CLIPPING_MASK 対応) */
const VERT_SHADER_SRC_BLEND = `\
...
/** ブレンドモード用フラグメントシェーダー本体 (main 関数を含む) */
const FRAG_SHADER_SRC_BLEND = `\
varying vec2 v_texCoord;
#ifdef CLIPPING_MASK
varying vec2 v_clipCoord;
uniform sampler2D s_texture1;
...
void main() {
    vec4 srcColor = texture2D(s_texture0, v_texCoord) * u_baseColor;

    // カラーブレンドとアルファブレンドを適用
    // (デフォルトの Normal / Over では実質的にパススルー)
    vec4 color = BlendColor(vec4(0.0), srcColor, 1.0);
    color = BlendAlpha(color, vec4(0.0), 1.0);
...
export class CubismShader_WebGL {
  ...
  public setupShaderProgramForDrawable(
    ...
    if (masked) {
      ...
      const clipCtx = renderer.getClippingContextBufferForDrawable()
      const tex: WebGLTexture = renderer
        .getDrawableMaskBuffer(clipCtx._bufferIndex)
        .getColorBuffer();
      ...
      this.gl.uniformMatrix4fv(
        ...
        false,
        clipCtx._matrixForDraw.getArray()
      );

      // 使用するカラーチャンネルを設定
      const channelIndex: number = clipCtx._layoutChannelIndex;
  ...
  public generateShaders(): void {
    ...
    // インラインシェーダーソースを使用（外部ファイル不要）
    this._vertShaderSrc = VERT_SHADER_SRC;
    this._vertShaderSrcMasked = VERT_SHADER_SRC_MASKED;
    this._vertShaderSrcSetupMask = VERT_SHADER_SRC_SETUP_MASK;
    this._fragShaderSrcSetupMask = FRAG_SHADER_SRC_SETUP_MASK;
    this._fragShaderSrcPremultipliedAlpha = FRAG_SHADER_SRC_PREMULTIPLIED_ALPHA;
    this._fragShaderSrcMaskPremultipliedAlpha = FRAG_SHADER_SRC_MASK_PREMULTIPLIED_ALPHA;
    this._fragShaderSrcMaskInvertedPremultipliedAlpha = FRAG_SHADER_SRC_MASK_INVERTED_PREMULTIPLIED_ALPHA;
    this._vertShaderSrcCopy = VERT_SHADER_SRC;
    this._fragShaderSrcCopy = FRAG_SHADER_SRC_PREMULTIPLIED_ALPHA;
    this._fragShaderSrcColorBlend = FRAG_SHADER_SRC_COLOR_BLEND_FUNCTIONS;
    this._fragShaderSrcAlphaBlend = FRAG_SHADER_SRC_ALPHA_BLEND_FUNCTIONS;
    this._vertShaderSrcBlend = VERT_SHADER_SRC_BLEND;
    this._fragShaderSrcBlend = FRAG_SHADER_SRC_BLEND;
  ...
  private generateBlendShader(
    ...
    for (
      ...
      // シェーダーコンパイル/リンク失敗時は skip (0 が返る)
      if (!this._shaderSets[shaderSetIndex].shaderProgram) {
        CubismLogError(
          'Failed to create blend shader program for colorBlend={0} alphaBlend={1} shaderType={2}',
          colorBlendMacro.trim(),
          alphaBlendMacro.trim(),
          shaderTypeIndex
        );
        continue;
      }
...
export class CubismShaderManager_WebGL {
  ...
  /**
   * 指定した GLContext に対応する Shader を解放し、マップから削除する。
   * Canvas の破棄時に呼び出してシングルトンのメモリリークを防ぐ。
   *
   * @param gl 解放対象の WebGL コンテキスト
   */
  public releaseShaderForContext(gl: WebGLRenderingContext): void {
